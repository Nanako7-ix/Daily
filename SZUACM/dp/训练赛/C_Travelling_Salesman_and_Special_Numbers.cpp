...
void solve() {
    ...
    m32 ans = 0;
    vector<int> cnt(1001);
    vector  dp(1000, vector (2, vector<m32> (1001)));
    vector vis(1000, vector (2, vector<int> (1001)));
    for (int i = 1; i <= 1000; ++i) {
        if (i != 1) cnt[i] = cnt[__builtin_popcount(i)] + 1;
        if (cnt[i] != n - 1) continue;
        ans += [&](this auto&& dfs, int p, bool lim, int need) -> m32 {
            if (need < 0 || need > p + 1) return 0;
            ...
            int Max = lim ? num[p] : 1;
            for (int i = 0; i <= Max; ++i) {
                dp[p][lim][need] += dfs(p - 1, lim && i == Max, need - i);
            }
            vis[p][lim][need] = 1;
            return dp[p][lim][need];
        } (999, 1, i);

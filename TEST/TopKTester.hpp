...
std::pair<std::vector<int>, u64> random_TopK(u64 n, int V = 1e9) {
...
// TestCase: 20
u64 testTopK(u64 n, int (*func)(std::vector<int>::iterator, std::vector<int>::iterator, u64 k), int V = 1e9) {
    i64 sum = 0;
    std::cout.flush();
    for(int T = 1; T <= 20; ++T) {
        auto [a, k] = random_TopK(n, V);

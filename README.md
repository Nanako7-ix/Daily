...
如何判断右端点是是否要移动？实际上直接使用点乘判断即可，如果使用叉乘找的话是错的，hack 也很简单：把上图的 D 点沿 $\overrightarrow{DG}$ 方向移动一下就 hack 了。
...
众所周知，如果 $p$ 是素数，且 $\gcd(a, p) = 1$，则 $a^p \equiv a \pmod p$，那么这条式子有这样的应用：

$$
a^{p-1} \equiv 1 \pmod p
$$

这表示当我们的幂次过大的时候，可以让指数对 $p - 1$ 取模，于是：
...
费马小定理在降幂上起到了显著的效果，但还是太多限制了。欧拉定理则解决了 $p$ 在非素数情况下的降幂问题：

如果 $\gcd(a, p) = 1$，那么 $a^{\varphi(p)} \equiv 1 \pmod p$。这里 $\varphi(n)$ 表示欧拉函数，欧拉函数 $\varphi(n)$ 表示 $[1, n]$ 中与 $n$ 互素的数字的个数。于是：

$$
a^{n} \equiv a^{n \bmod \varphi(p)} \pmod p
$$

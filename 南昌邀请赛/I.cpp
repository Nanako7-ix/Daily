...
void solve() {
    ...
    m32 ans = 0;
    for (int l = 1, r = k; r <= cnt[n]; l++, r++) {
        int L = lower_bound(cnt.begin(), cnt.end(), l - 1) - cnt.begin() + 1;
        int R = upper_bound(cnt.begin(), cnt.end(), r) - cnt.begin() - 1;
        int len = R - L + 1;
        ans += comb.C(len, k);
        if (l != 1) {
            R = lower_bound(cnt.begin(), cnt.end(), r) - cnt.begin() - 1;
            len = R - L + 1;
            ans -= comb.C(len, k - 1);
        }
    }

...
template<auto Mod> requires std::integral<decltype(Mod)>
using MInt = ModInt<
            std::conditional_t<(Mod <= std::numeric_limits<u32> ()), u32, u64>,
static_cast<std::conditional_t<(Mod <= std::numeric_limits<u32> ()), u32, u64>>(Mod)
>;
...
void Thephix() {
  ...
  vector<vector<Z>> dp(n + 2, vector<Z> (n));
  dp[1][0] = 1, dp[2][0] = -1;
  for (int i = 1; i <= n; ++i) {
    for (int j = 0; j < n; ++j) {
      dp[i][j] += dp[i - 1][j];
    ...
    int l = i + 1, r = i + a[i] + 1;
    for (int j = 0; j + 1 < n; ++j) {
      dp[l][j + 1] += dp[i][j] * inv[a[i]];
      dp[r][j + 1] -= dp[i][j] * inv[a[i]];
    }
  }
  Z ans = 0;
  for (int i = 0; i < n; ++i) {

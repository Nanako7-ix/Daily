...
bitset<N * 24> f;
int lc[N * 24], rc[N * 24];
bool update(int lu, int &u, int L, int R, int x) {
    u = ++tot, f[u] = f[lu], lc[u] = lc[lu], rc[u] = rc[lu];
    if (L == R) {
        if(f.test(lu)) return false;
        f.set(u);
        return true;
    }
    int M = L + R >> 1;
    if (x <= M)
        return update(lc[lu], lc[u], L, M, x);
    else
        return update(rc[lu], rc[u], M + 1, R, x);
}
int root[N];
int main() {
    ...
    while (n--) {
        ...
        if (c == '+') {
            cin >> x;
            ++cnt;
            fa[0][cnt] = now, now = cnt;
            for (int i = 1; i <= 10; i++)
                fa[i][now] = fa[i - 1][fa[i - 1][fa[i - 1][fa[i - 1][now]]]];
            ans[now] = ans[fa[0][now]] + update(root[fa[0][now]], root[now], 1, 1000000, x);
            lst[++top] = now;
        } else if (c == '-') {
            cin >> x;
            for (int i = 10; ~i; i--)
                while (x >= (1 << (i << 1)))
                    now = fa[i][now], x -= (1 << (i << 1));
            lst[++top] = now;
        } else if (c == '!')
            now = lst[--top]; // cout << "cmkakioi" << endl;
        else if (c == '?')
            cout << ans[now] << endl;
        // cout << "now:" << now << ' ' << top <<' ' << ans[now] << endl;
    }
    return 0;
}
...
void solve() {
    ...
    vector<int> lft(n + 1, 1), rgt(n + 2, n);
    for(int i = 1; i <= n; ++i) {
        if(a[i] == 0) lft[i] = i;
        else lft[i] = lft[i - 1];
    }
    ...
    LazySegmentTree<Info, Info> dp(n + 1);
    dp.modify(1, [&](Info& info) {info.val = 0;});
    for(int i = 1; i <= n; ++i) {
        if(a[i] == 0) {
            dp.modify(i + 1, i + 1, dp.query(i, i).val + 1);
        } else if(a[i] == 1) {
            dp.modify(i + 1, i + 1, dp.query(lft[i], lft[i]).val + 1);
            dp.modify(i + 1, rgt[i] + 1, dp.query(i, i) + 1);
        } else if(a[i] == 2) {
            dp.modify(i + 1, rgt[i] + 1, dp.query(lft[i], lft[i]).val + 1);
        }
    }
    cout << dp.query(n + 1, n + 1).val << endl;
}

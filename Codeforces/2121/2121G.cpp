...
Info operator+ (const Info& x, const Info& y) {
    return Info { x.cnt + y.cnt, x.sum + y.sum };
}
...
void solve() {
    ...
    for (int i = 0, cnt = 0; i < n; ++i) {
        cnt += s[i] == '0' ? 1 : -1;
        seg.modify(p + cnt, [&](Info& info) {
            info.cnt++;
            info.sum += max((i + 1 + cnt) / 2, (i + 1 - cnt) / 2);
        });
    }

    i64 ans = 0;
    for (int i = 0; i < n; ++i) {
        ans += seg.query(1, 2 * n + 1).sum;
        int cnt = s[i] == '0' ? 1 : -1;
        seg.modify(p + cnt, [&](Info& info) {
            info.cnt--, info.sum--;
        });
        if (s[i] == '0' && 1 + p <= 2 * n + 1) {
            seg.modify(1 + p, 2 * n + 1, -1);
        }
        if (s[i] == '1' && 1 <= p - 1) {
            seg.modify(1, p - 1, -1);
        }

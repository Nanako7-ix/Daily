...
void solve() {
    ...
    vector vis(n + 1, vector (n + 1, vector<int> (n + 1)));
    vector dp(n + 1, vector (n + 1, vector<m32> (n + 1)));
    auto dfs = [&](auto dfs, int i, int j, int k) -> m32 {
        int ii = n - i + 1;
        assert(i <= ii);
        if (j < 1 || j > i || k < 1 || k > ii || k - j > ii - i || k < j || a[i][j] != a[ii][k]) {
            return 0;
        } else if (ii - i <= 1) {
            return 1;
        } else if (!vis[i][j][k]) {
            dp[i][j][k] += dfs(dfs, i + 1, j, k - 1);
            dp[i][j][k] += dfs(dfs, i + 1, j + 1, k - 1);
            dp[i][j][k] += dfs(dfs, i + 1, j, k);
            dp[i][j][k] += dfs(dfs, i + 1, j + 1, k);
            vis[i][j][k] = 1;
        }
        return dp[i][j][k];
    };
